...
namespace Practice.Models.Helpers
{
    class UserValidator
    {
        ...
        {
            if (!Regex.IsMatch(user.Login, "^[a-zA-Z][a-zA-Z0-9_]{3,11}$"))
            {
                throw new InvalidDataException("Incorrect entered login." +
                    "\nmust be 4-12 symbols, eng language, first a letter, only letters, digits, and symbol _");
            }
        }
        private void CheckPassword()
        {
            if (!Regex.IsMatch(user.Password, "^[a-zA-Z0-9_]{4,12}$"))
            {
                throw new InvalidDataException("Incorrect entered password." +
                    "\nmust be 4-12 symbols, eng language, only letters, digits, and symbol _");
            }
        }
    }
}
